...
export class Player {
  constructor(x, y, context, movementLimites) {
    this.velocity = 3;
    this.radius = 15;
    ...
    this.movementLimites = {
      minX: movementLimites.minX + this.radius,
      maxX: movementLimites.maxX - this.radius,
      minY: movementLimites.minY + this.radius,
      maxY: movementLimites.maxY - this.radius,
    };
  ...
  checkPositionLimitAndUpdate() {
    if (this.y < this.movementLimites.minY) this.y = this.movementLimites.minY;
    if (this.y > this.movementLimites.maxY) this.y = this.movementLimites.maxY;
    if (this.x < this.movementLimites.minX) this.x = this.movementLimites.minX;
    if (this.x > this.movementLimites.maxX) this.x = this.movementLimites.maxX;
  }
